...
export function Footer() {
  ...
  return (
    <footer className="mt-12 py-8 px-4 bg-gradient-to-br from-[#F8F9FA] to-[#E9ECEF] rounded-t-3xl">
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-6">
          {links.map((link) => (
            <a
              key={link.id}
              href={link.href}
              className="text-muted-foreground hover:text-[#9C6FFF] transition-colors duration-300 min-h-[44px] flex items-center"
            >
              {link.label}
            </a>
          ))}
